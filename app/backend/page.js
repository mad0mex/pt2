...
export default function BackendOverview() {
  ...
  const getFieldMap = (productKey) => {
    return FORM_FIELDS[productKey]?.reduce((acc, field) => {
          ...
          <div key={prod.id} className="border rounded-lg p-4 bg-amber-100 shadow">
            ...
            {prod.zutaten.length > 0 ? (
              <table className="w-full table-auto border-collapse mt-3 text-sm">
                ...
                <tbody>
                {(() => {
                    const fieldMap = getFieldMap(prod.template_key);
                    return prod.zutaten.map((zutat, idx) => (
                      <tr key={idx}>
                        <td className="border px-2 py-1">
                          {fieldMap[zutat.zutat] || zutat.zutat}
                        </td>
        ...
        <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-amber-100 border-3 p-6 rounded shadow-xl w-[90%] max-w-3xl">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => setEditingProd(null)} className="bg-gray-300 hover:bg-gray-400 transition px-4 py-2 rounded">Abbrechen</button>
            </div>
          </div>
        </div>
      )}
